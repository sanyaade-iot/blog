...
namespace :site do
  ...
  task :publish => [:genprod] do
    Dir.mktmpdir do |tmp|
      ...
      system "git remote add origin https://zsup:#{ENV['GH_TOKEN']}@github.com:#{GITHUB_REPONAME}.git"
  ...
  task :staging => [:genstage] do
    Dir.mktmpdir do |tmp|
      ...
      message = "Site updated at #{Time.now.utc}"
      system "git commit -m #{message.inspect}"
      system "git remote add origin https://zsup:#{ENV['GH_TOKEN']}@github.com:#{STAGING_REPONAME}.git"
      system "git push origin master:refs/heads/gh-pages --force"
    end
  end
end
